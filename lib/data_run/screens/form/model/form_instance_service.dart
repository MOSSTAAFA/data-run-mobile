...
class FormInstanceService {
  ...
  // Future<Map<String, AbstractControl<dynamic>>> _formAttributeControls() async {
  //   DUser? currentUser = await D2Remote.userModule.user.getOne();
  //
  //   final attributeControls = <String, AbstractControl<dynamic>>{
  //     AttributeType.uuid.name: FormControl<String>(value: _uuid),
  //     AttributeType.username.name:
  //         FormControl<String>(value: currentUser?.username),
  //     AttributeType.form.name: FormControl<String>(value: formMetadata.form),
  //     AttributeType.orgUnit.name: FormControl<String>(value: orgUnit),
  //     AttributeType.team.name: FormControl<String>(value: formMetadata.team),
  //     AttributeType.activity.name:
  //         FormControl<String>(value: formMetadata.activity),
  //     AttributeType.version.name: FormControl<int>(value: formMetadata.version)
  //   };
  //
  ...
  // Future<Map<String, FormElementInstance<dynamic>>>
  //     formAttributeElements() async {
  //   final Map<String, FormElementInstance<dynamic>> attributeElements = {};
  //
  //   final attributesControls = await _formAttributeControls();
  //
  //   for (var element in template.fields) {
  //     if (!element.type.isSelectType && element.mainField) {
  //       attributeElements[element.name] = FromElementFactory.createElement(
  //           FormGroup(attributesControls), element,
  //           formOptionsMap: formOptionsMapCache);
  //     }
  //   }
  ...
  // Future<FormGroup> _form() async {
  //   final attributesControls = await _formAttributeControls();
  //   final formControls = await _formDataControls();
  //
  //   return FormGroup({
  //     formAttributesGroupName: FormGroup(attributesControls),
  //     formDataGroupName: FormGroup(formControls)
  //   });
  // }
  ...
  Future<Map<String, FormElementInstance<dynamic>>> formDataElements(
    ...
    for (var element in template.fields) {
      elements[element.name] = FromElementFactory.createElement(form, element,
          formOptionsMap: formOptionsMapCache);
    }
