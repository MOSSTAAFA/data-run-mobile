...
Map<String, dynamic> flattenElement(FormElementInstance element) {
  ...
  if (element is FormFieldElement) {
    // Simple field extraction
    result[element.name!] = element.value;
  } else if (element is FormSectionElement) {
...
/// Now, to deserialize (rehydrate) the flattened data back into the structured FormInstance
//
//     Parse the Form Template: Use the form template to identify the sections, fields, and repeatable sections.
//
//     Iterate Over Flattened Data: For each flattened entry (e.g., section1.field2 or repeatableSection1.0.section2.field3), traverse the form template and place the value in the corresponding section/field object.
//
//     Rebuild Form Structure: Based on the template, rebuild the FormInstance, SectionInstance, and RepeatableSectionInstance objects.
///
FormElementInstance<dynamic> deserializeFlattenedData(
  ...
  formTemplate.fields.forEach((field) {
    ...
    if (!field.type.isRepeatSection) {
      Map<String, FormFieldElement> sectionFields = {};
      for (var childField in field.fields) {
        ...
        if (flattenedData.containsKey(flattenedKey)) {
          sectionFields[childField['name']] = FormFieldElement(
      ...
      formElements.add(FormSectionElement(
        name: field['name'],
    ...
    // Handle repeatable sections
      ...
      List<FormSectionElement> repeatedSections = [];
      int i = 0;
      while (true) {
        Map<String, FormFieldElement> sectionFields = {};
        for (var fieldTemplate in field['sections'][0]['fields']) {
          ...
          if (flattenedData.containsKey(flattenedKey)) {
            sectionFields[fieldTemplate['name']] = FormFieldElement(
        ...
        repeatedSections.add(FormSectionElement(
